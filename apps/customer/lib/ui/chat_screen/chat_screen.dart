  ...
  getOrder() async {
    if (widget.orderId != null) {
      if ((widget.type == 'vendor_chat' ||
              widget.type?.toLowerCase() == 'restaurant' ||
              widget.chatType == 'vendor_chat') &&
          widget.chatType != 'Driver') {
        OrderModel orderModel =
            (await FireStoreUtils().getOrderById(widget.orderId ?? ''))!;
        VendorModel vendor =
            await FireStoreUtils().getVendorByVendorID(orderModel.vendorID);
        isActiveChat = ((isSubscriptionModelApplied == false &&
            ...
            ((isSubscriptionModelApplied ||
                ...
                vendor.subscriptionPlan?.features?.chat == true));
      } else if ((widget.type == 'provider_chat' ||
              widget.chatType == 'Provider') &&
          widget.chatType != 'Worker') {
        OnProviderOrderModel providerOrderModel = (await FireStoreUtils()
            .getProviderOrderById(widget.orderId ?? ''))!;
        ProviderServiceModel? providerServiceModel =
            await FireStoreUtils.getCurrentProvider(
                providerOrderModel.provider.id!);
        isActiveChat = ((isSubscriptionModelApplied == false &&
                sectionConstantModel?.adminCommision?.enable == false) ||
            ((isSubscriptionModelApplied ||
                    sectionConstantModel?.adminCommision?.enable == true) &&
                providerServiceModel?.subscriptionPlan?.features?.chat ==
                    true));
      } else {
        isActiveChat = widget.isChatEnable;
      }
    } else {
      isActiveChat = widget.isChatEnable;
    }
                          ...
                          onPressed: () async {
                            if (isActiveChat == false) {
                              ShowToastDialog.showToast(
                                  "Chat feature has been disabled by the receiver."
                                      .tr());
                            } else {
                              _onCameraClick();
                            }
