...
class _OnDemandHomeScreenState extends State<OnDemandHomeScreen> {
  ...
  Future<void> getData() async {
    ...
    List<String?> listofuniqueprovider =
        uniqueAuthorIds.toList(); // get all unique provider
    ...
    for (var provider in listofuniqueprovider) {
      List<ProviderServiceModel> filteredList =
          providerServiceList.where((service) {
      ...
      for (int index = 0; index < filteredList.length; index++) {
        if (isSubscriptionModelApplied == true ||
          ...
          if (filteredList[index].subscriptionPlan?.itemLimit == "-1") {
            ...
            if (index <
                int.parse(
                    filteredList[index].subscriptionPlan?.itemLimit ?? '0')) {
              providerList.add(filteredList[index]);
            }
          }
        } else {
                            ...
                            child: Row(
                              children: [
                                ...
                                InkWell(
                                  onTap: () {
                                    push(
                                        context, ViewAllPopularServiceScreen());
                                  },
                                  child: Text(
                                    "View all".tr(),
                                    textAlign: TextAlign.center,
                            ...
                            widget.providerList.disPrice == "" ||
                                ...
                                ? Text(
                                    widget.providerList.priceUnit == 'Fixed'
                                        ...
                                        : '${amountShow(amount: widget.providerList.price ?? '0')}/hr',
                                    style: TextStyle(
                                      fontSize: 14,
                                      fontFamily: AppThemeData.regular,
                                      fontWeight: FontWeight.bold,
