  ...
  */
      ...
      if (userDocument.data() != null && userDocument.exists) {
        try {
          ...
          await firestore.collection(USERS).doc(userId).update({
            ...
            MyAppState.currentUser!.walletAmount =
                num.parse(userDocument.data()!['wallet_amount'].toString()) +
                    amount;
          });
        } catch (error) {
  ...
  static Future updateUserCate(String userid, String sectionid) async {
    return await firestore
        .collection(USERS)
        .doc(userid)
        .update({"section_id": sectionid}).then((document) {
      return;
    });
  }
  ...
  Future<List<ProductModel>> getProductsFuture(String vendorID) async {
    ...
    await firestore
      ...
      if (value.docs.isNotEmpty) {
        for (var element in value.docs) {
          ProductModel subscriptionPlanModel =
              ProductModel.fromJson(element.data());
          productList.add(subscriptionPlanModel);
        }
      }
    });
    return productList;
  ...
  static firebaseSignUpWithEmailAndPassword(
      ...
      String lastName,
      String mobile,
      bool? autoApproveRestaurant) async {
    try {
      auth.UserCredential result = await auth.FirebaseAuth.instance
  ...
  static Future restaurantVendorWalletSet(OrderModel orderModel) async {
    ...
    for (var element in orderModel.products) {
      log("discount_price :: ${element.discount_price.toString()}");
      if (double.parse(element.discount_price.trim().isEmpty
              ? '0'
    ...
    TopupTranHistoryModel taxModel = TopupTranHistoryModel(
        amount: taxAmount,
        id: Uuid().v4(),
        orderId: orderModel.id,
        userId: orderModel.vendor.author,
    ...
    await firestore
        ...
        .doc(historyModel.id)
        .set(historyModel.toJson());
    await firestore.collection(Wallet).doc(taxModel.id).set(taxModel.toJson());
    await updateVendorWalletAmount(
        amount: basePrice + taxAmount, userId: orderModel.vendor.author);
  }
  ...
  Future updateWallateAmountEcommarce(OrderModel orderModel) async {
    ...
    for (var element in orderModel.products) {
      log("element.discount_price :: ${element.discount_price}");
      if (double.parse(
              element.discount_price == '' ? '0' : element.discount_price) <=
          0) {
        subTotal = subTotal +
            double.parse(element.price.toString()) *
                double.parse(element.quantity.toString()) +
    ...
    if (vendorAdminCommission!.enable == true) {
      basePrice =
          (subTotal / (1 + (double.parse(orderModel.adminCommission!) / 100))) -
              double.parse(orderModel.discount.toString()) -
              specialDiscount;
  ...
  static Future<MercadoPagoModel?> getMercadoPagoSettingData() async {
    ...
    await firestore
        ...
        .doc("MercadoPago")
        .get()
        .then((mercadoPago) async {
      try {
        mercadoPagoDataModel = MercadoPagoModel.fromJson(mercadoPago.data()!);
        await Preferences.setString(Preferences.mercadoPagoPref,
            jsonEncode(mercadoPagoDataModel?.toJson()));
      } catch (error) {
        debugPrint(error.toString());
      }
    });
  ...
  static Future<PayPalModel?> getPaypalSettingData() async {
    ...
    await firestore
        ...
        .doc("paypalSettings")
        .get()
        .then((paypalData) async {
      try {
        paypalDataModel = PayPalModel.fromJson(paypalData.data()!);
        await Preferences.setString(
            Preferences.paypalPref, jsonEncode(paypalDataModel?.toJson()));
      } catch (error) {
        debugPrint(error.toString());
      }
    });
  ...
  static Future<StripeSettingData?> getStripeSettingData() async {
    ...
    await firestore
        ...
        .doc("stripeSettings")
        .get()
        .then((stripeData) async {
      try {
        stripeSettingData = StripeSettingData.fromJson(stripeData.data()!);
        await Preferences.setString(
            Preferences.stripePref, jsonEncode(stripeSettingData?.toJson()));
      } catch (error) {
        debugPrint(error.toString());
      }
    });
  ...
  static Future<FlutterWaveModel?> getFlutterWaveSettingData() async {
    ...
    await firestore
        ...
        .doc("flutterWave")
        .get()
        .then((flutterWaveData) async {
      try {
        flutterWaveSettingData =
            FlutterWaveModel.fromJson(flutterWaveData.data()!);
        await Preferences.setString(Preferences.flutterwavePref,
            jsonEncode(flutterWaveSettingData?.toJson()));
      } catch (error) {
        debugPrint("error>>>122");
        debugPrint(error.toString());
      }
  ...
  static Future<PayStackModel?> getPayStackSettingData() async {
    ...
    await firestore
        ...
        .doc("payStack")
        .get()
        .then((payStackData) async {
      try {
        payStackSettingData = PayStackModel.fromJson(payStackData.data()!);
        await Preferences.setString(Preferences.paystackPref,
            jsonEncode(payStackSettingData?.toJson()));
      } catch (error) {
        debugPrint("error>>>122");
        debugPrint(error.toString());
      }
  ...
  static Future<PaytmModel?> getPaytmSettingData() async {
    ...
    await firestore
        ...
        .doc("PaytmSettings")
        .get()
        .then((paytmData) async {
      try {
        paytmSettingData = PaytmModel.fromJson(paytmData.data()!);
        await Preferences.setString(
            Preferences.paytmPref, jsonEncode(paytmSettingData?.toJson()));
      } catch (error) {
        debugPrint(error.toString());
      }
    });
  ...
  static Future<WalletSettingModel?> getWalletSettingData() async {
    WalletSettingModel? wallet;
    await firestore
        .collection(Setting)
        .doc('walletSettings')
        .get()
        .then((walletSetting) async {
      try {
        wallet = WalletSettingModel.fromJson(walletSetting.data()!);
        await Preferences.setString(
            Preferences.walletPref, jsonEncode(wallet?.toJson()));
      } catch (e) {
  ...
  static Future<OrangeMoney?> getOrangeMoneySettingData() async {
    ...
    await firestore
      ...
      try {
        ...
        await Preferences.setString(Preferences.orangeMoneyPref,
            jsonEncode(payStackSettingData?.toJson()));
      } catch (error) {
  ...
  static Future<Xendit?> getXenditSettingData() async {
    ...
    await firestore
      ...
      try {
        ...
        await Preferences.setString(
            Preferences.xenditPref, jsonEncode(payStackSettingData?.toJson()));
      } catch (error) {
        print(error.toString());
      }
    });
  ...
  static Future<MidTrans?> getMidTransSettingData() async {
    ...
    await firestore
      ...
      try {
        ...
        await Preferences.setString(Preferences.midTransPref,
            jsonEncode(payStackSettingData?.toJson()));
      } catch (error) {
        print(error.toString());
      }
  ...
  static Future<RazorPayModel?> getRazorPayDemo() async {
    ...
    await firestore
      ...
      try {
        ...
        await Preferences.setString(
            Preferences.razorpayPref, jsonEncode(userModel?.toJson()));
      } catch (e) {
        debugPrint(
            'FireStoreUtils.getUserByID failed to parse user object ${user.id}');
      }
  ...
  static Future<List<TopupTranHistoryModel>?> getFilterWalletTransaction(
      Timestamp startTime, Timestamp endTime) async {
    List<TopupTranHistoryModel> walletTransactionList = [];
    await firestore
        .collection(Wallet)
        .where('user_id', isEqualTo: MyAppState.currentUser!.userID)
        ...
        .then((value) {
      for (var element in value.docs) {
        TopupTranHistoryModel walletTransactionModel =
            TopupTranHistoryModel.fromJson(element.data());
  ...
  static Future<List<TopupTranHistoryModel>?> getWalletTransaction() async {
    List<TopupTranHistoryModel> walletTransactionList = [];
    await firestore
        .collection(Wallet)
        .where('user_id', isEqualTo: MyAppState.currentUser!.userID)
  ...
  static Future<List<WithdrawHistoryModel>?> getWithdrawHistory() async {
    List<WithdrawHistoryModel> walletTransactionListdata = [];
    await firestore
        .collection(Payouts)
        .where('vendorID',
            isEqualTo: MyAppState.currentUser?.vendorID.toString())
        .orderBy('paidDate', descending: true)
        .get()
        .then((value) {
      for (var element in value.docs) {
        WithdrawHistoryModel walletTransactionModel =
            WithdrawHistoryModel.fromJson(element.data());
        walletTransactionListdata.add(walletTransactionModel);
      }
    }).catchError((error) {
      log(error.toString());
    });
    return walletTransactionListdata;
  }
}
