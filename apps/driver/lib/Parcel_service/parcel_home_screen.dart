      ...
      if (currentOrder != null) {
        if (currentOrder!.status == ORDER_STATUS_SHIPPED) {
          ...
          osmflutter.GeoPoint destinationLocation =
              osmflutter.GeoPoint(latitude: currentOrder?.receiverLatLong?.latitude ?? 0.0, longitude: currentOrder?.receiverLatLong?.longitude ?? 0.0);
          setOsmMarker(departure: sourceLocation, destination: destinationLocation);
          roadInfo = await mapOsmController.drawRoad(
            sourceLocation,
            destinationLocation,
          ...
          osmflutter.GeoPoint sourceLocation =
              osmflutter.GeoPoint(latitude: currentOrder?.senderLatLong?.latitude ?? 0.0, longitude: currentOrder?.senderLatLong?.longitude ?? 0.0);
                        ...
                        Padding(
                          ...
                          child: SizedBox(
                            ...
                            child: Text(
                              ...
                              overflow: TextOverflow.ellipsis,
                              style:
                                  TextStyle(fontSize: 12, color: isDarkMode(context) ? Color(0xffFFFFFF) : Color(0xff000000), fontFamily: AppThemeData.regular, letterSpacing: 0.5),
                            ),
                          ),
                        ),
                      ],
                        ...
                        Padding(
                          ...
                          child: SizedBox(
                            ...
                            child: Text(
                              ...
                              overflow: TextOverflow.ellipsis,
                              style:
                                  TextStyle(color: isDarkMode(context) ? Color(0xffFFFFFF) : Color(0xff000000), fontSize: 12, fontFamily: AppThemeData.regular, letterSpacing: 0.5),
                            ),
                          ),
                        ),
                      ],
        ...
        if (currentOrder!.paymentMethod.toLowerCase() != "cod") {
          ...
          if (currentOrder!.taxModel != null) {
            for (var element in currentOrder!.taxModel!) {
              totalTax = totalTax +
                  calculateTax(amount: (double.parse(currentOrder!.subTotal!.toString()) - double.parse(currentOrder!.discount!.toString())).toString(), taxModel: element);
            }
          }
