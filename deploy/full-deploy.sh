...
echo ""
echo "Comando único pós-deploy (produção + validação):"
echo "  cd $REPO_CLONE/deploy && sudo ./set-production.sh && sudo ./check-env.sh"
echo ""
...
echo "  3. Rode set-production.sh + check-env.sh (DEBUG=false, timezone, LOCALE)"
...
echo "  6. Opcional: ./backup-mysql.sh  |  hardening: scripts/harden-reminders.md"
