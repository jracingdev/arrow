...
<script type="text/javascript">
    ...
    async function getOrderDetails() {
        bookingRef.get().then(async function(bookingSnapshots) {
            ...
            if (orderDetails.authorID != user_uuid) {
                ...
                $("#booking-status").html('<span class="py-2 px-3 order_rejected">' + ArrowI18n.status(order_status) + '<span>');
    ...
    function formatTime(timeString) {
        return (window.ArrowDateTime && ArrowDateTime.formatClock)
            ? ArrowDateTime.formatClock(timeString)
            : timeString;
    }
