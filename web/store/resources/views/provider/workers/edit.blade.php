...
<div class="page-wrapper">
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h3 class="text-themecolor">{{ trans('lang.worker_edit') }}</h3>
        </div>
        <div class="col-md-7 align-self-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ trans('lang.dashboard') }}</a></li>
                <li class="breadcrumb-item"><a href="{{ route('provider.workers') }}">{{ trans('lang.worker_plural') }}</a></li>
                <li class="breadcrumb-item active">{{ trans('lang.worker_edit') }}</li>
            </ol>
        </div>
    </div>
    <div class="card-body">
        ...
        <div class="row vendor_payout_create">
            <div class="vendor_payout_create-inner">
                <fieldset>
                    ...
                    <div class="form-group row width-50">
                        <label class="col-3 control-label">{{ trans('lang.rating') }}</label>
                        <div class="col-7"><input type="text" class="form-control" id="rating" readonly></div>
                    </div>
...
<script>
    ...
    $(function () {
        ...
        database.collection('providers_workers').doc(id).get().then(function (snap) {
            ...
            photoUrl = w.profilePictureURL || '';
            $('#firstName').val(w.firstName || '');
            $('#lastName').val(w.lastName || '');
            $('#email').val(w.email || '');
            $('#phone').val(w.phoneNumber || w.phone || '');
            $('#salary').val(w.salary || '');
            $('#address').val(w.address || '');
            $('#latitude').val(w.latitude || '');
            $('#longitude').val(w.longitude || '');
            $('#online').prop('checked', w.online === true);
            $('#rating').val(formatRating(w.reviewsSum, w.reviewsCount));
            if (photoUrl) {
        ...
        $('#save_btn').on('click', function () {
            ...
            if (!firstName || !lastName || !email || !phone || !salary || isNaN(lat) || isNaN(lng)) {
