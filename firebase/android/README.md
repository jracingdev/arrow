...
| Prestador | `br.app.arrow.provider` | **criar no console** (ainda não existe) | `apps/provider/android/app/google-services.json` — entregar à Frente 1 |

## App Android novo: prestador (`br.app.arrow.provider`)

O `google-services.json` atual **não** tem client `br.app.arrow.provider`. Não inventar App ID. Passos no Console **j-arrow** (número `661081769489`):
...
8. Entregar o JSON à **Frente 1** (`apps/provider/android/app/google-services.json`). Também atualizar o App ID em `ArrowFirebaseAndroidAppIds.provider` (hoje placeholder `CRIAR_NO_CONSOLE`).
